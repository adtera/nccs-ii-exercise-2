...
def calculate_distances_within_treshold_radii(configurations, rs, side_length):
    # Calculate distances bigger than threshold radius ri
    distances_within_ri_all_timesteps = 0
    for t, timestep_config in enumerate(configurations):
        print(f"timestep: {t}")
        ...
        for d in distances:
            ...
            vals.append(
                list(
                    map(act_on_iter,
                        (lambda x: x < rs[0], lambda x: x < rs[1],
                         lambda x: x < rs[2], lambda x: x < rs[3],
                         lambda x: x < rs[4], lambda x: x < rs[5],
                         lambda x: x < rs[6], lambda x: x < rs[7],
                         lambda x: x < rs[8], lambda x: x < rs[9]), [d] * 10)))

        vals = np.array(vals)
        vals_absolutes = vals.sum(axis=0)
        print(np.array(vals).shape)
        print(np.array(vals).sum(axis=0))

        #print(count_d_bigger_ri)
        #        fraction_of_distances_within_ri =  count_d_smaller_ri/len(distances)
        fraction_of_distances_within_ri = vals_absolutes / len(distances)
        #print(fraction_of_distances_bigger_ri)
        distances_within_ri_all_timesteps += fraction_of_distances_within_ri
...
with open('./ratio_distances_within_radii_dev2.txt', 'w') as out_file:
    out_file.write("  ".join([
        "Threshold radii ", "Fraction of distances within radius over time"
    ]) + "\n")
    ratios = calculate_distances_within_treshold_radii(
        configuration_timesteps_75, rs, L)
    for r in range(len(ratios)):
        out_file.write("   ".join([str(rs[r]), str(ratios[r])]) + '\n')
